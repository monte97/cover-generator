...
for (const file of fs.readdirSync(TEMPLATES_DIR)) {
    ...
    const nameFromComment = extractName(source);

    if (!nameFromComment) {
        console.warn(`⚠️  Template "${file}" has no name comment — using filename as display name`);
    }

    const displayName = nameFromComment || id;
